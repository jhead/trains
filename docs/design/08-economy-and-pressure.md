...
**Every running cost is per _real_ minute — the clock the player is sitting at.** The world runs 640× faster than the wall, and the two minutes are easy to confuse in a way that is not cosmetic: read as sim-minutes, every authored rate collects at 1/640 of its stated value, upkeep lands at about 3% of gross income, one train pays for five thousand tiles of dead track, and this section's trap cannot occur at all. That is not hypothetical — it shipped, and it is why the status strip, the ledger and every constant behind them are stated in the minutes the player is living in. [17 — Time & Pacing](17-time-and-pacing.md) is the standard; §1 is the table of which clock each thing speaks.

**A fare is paid per journey, and that is what makes the timetable an economic constant.** Costs are per real minute, so how many journeys a train fits into one is on the income side of every ratio in this brief. When 17 §4 halved train speed for watchability, the per-tile fare and the per-tile goods payout both doubled — not a buff, a re-denomination, and the measured `$/min` either side of it is the same to within a percent. Anyone moving train speed again owes the same arithmetic, and `rail_sim/tests/economy_cold_start.rs` is where it is checked.
