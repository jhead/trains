...
| **Train acceleration** (capacity retired) | Capacity is built: a transit couples up to three cars, each carrying one load, priced at half a train and gated by whether a pair's queue is deeper than one carriage (07 §3.1–3.3). **Acceleration is not, and the consist penalty rides on the wrong field** — it lifts `base_ticks`, so a long train is slower *everywhere* rather than slower away from a stop, which is where 07 §3 wants the difference to bite. Freight capacity is deliberately parked at one wagon until industries carry stock (07 §3.4), so half of the capacity row is a decision rather than a gap. |
...
| **The projection globals are owned on write, not on read** | A test build now refuses a write to the live projection or the iso height field from a thread that does not hold `map::tests::ProjectionGuard`, which is what killed the `an_anchored_sprite_follows_a_change_of_world` race. Two residuals. **Reads are still unguarded**: nothing stops a test that only calls `tile_to_world` / `tile_height` from reading a projection some *other* test installed. The window is far smaller than it was — every writer now restores on drop, so the globals are only ever off-baseline inside a guard's critical section — but it is not nil, and closing it means a guard on nearly every test in the crate, which is more change than the bug is worth. **And the check sits at `map::projection`'s three wrappers, not at `rail_map`'s setters**, so a future call site that reaches past them to `rail_map::set_projection` directly is unchecked. That placement is deliberate and the alternative was tried: asserting inside `rail_map` needs a Cargo feature, Cargo unions `dev-dependencies` features into the *one* `rail_map` a `cargo test` invocation builds, and that invocation also builds the plain binary — which left `target/debug/rail_town` panicking at boot on the ownership message. A missing wrapper is a quiet gap; a game that will not start is a loud one, every day. `map::tests::writing_the_globals_without_owning_them_is_refused` is the tripwire under what remains. |
