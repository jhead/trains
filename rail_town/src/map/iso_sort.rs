...
//! and none of the behaviour, so instead this runs once in `PostUpdate` — after
...
mod tests {
    ...
    /// The sorter only runs in isometric, and it reads the tile under a sprite
    /// back out of the projection, so a test has to install it.
