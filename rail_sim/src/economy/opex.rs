...
pub fn apply_train_opex(mut money: ResMut<Money>, mut q: Query<&mut TrainLocation>) {
    for mut loc in q.iter_mut() {
        if loc.parked {
            // Try to unpark if we can afford opex again.
            if money.can_afford(TRAIN_OPEX_CENTS) {
                if money.try_debit(TRAIN_OPEX_CENTS).is_ok() {
                    loc.parked = false;
                }
            }
            continue;
        }
        if money.try_debit(TRAIN_OPEX_CENTS).is_err() {
            loc.parked = true;
        }
    }
}
